...
class CoverageAnalyzer:
    ...
    def __init__(self, base_dir: Path):
        ...
        self.commit_mining_dir = base_dir.parent / 'commit_mining'

    def _find_repo_path(self, repo_name: str) -> Path:
        """Find the repository path by searching in all framework directories"""
        frameworks = ['pytorch', 'tensorflow', 'matplotlib']
        for framework in frameworks:
            repo_path = self.commit_mining_dir / framework / 'repos' / repo_name
            if repo_path.exists():
                return repo_path
        return None
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Path to aggregated test mapping CSV (default: test_mappings/aggregated_test_mapping.csv)'
    )
    parser.add_argument(
        '--output',
        type=str,
        default=None,
        help='Path to output CSV (default: coverage_analysis.csv in script directory)'
    )
    args = parser.parse_args()

    script_dir = Path(__file__).parent
    if args.input:
        test_mapping_file = Path(args.input)
    else:
        test_mapping_file = script_dir / 'test_mappings' / 'aggregated_test_mapping.csv'
    
    if args.output:
        output_file = Path(args.output)
    else:
        output_file = script_dir / 'coverage_analysis.csv'

    if not test_mapping_file.exists():
        print(f"Error: {test_mapping_file} not found")
        print("Please run aggregate_test_mappings.py first")
        return 1
